...
for i=1:numPeople,
    if (pedigree.parents(i, 1) == 0 && pedigree.parents(i, 2) == 0),
        ...
        j = pedigree.parents(i, 1);
        k = pedigree.parents(i, 2);
        factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, j, numPeople+j);
        factorList(numPeople+i) = childCopyGivenParentalsFactor(numAlleles, numPeople+i, k, numPeople+k);
    end
